...
def load_data():
    ...
    write_to_db('promoted_content', data, header)
...
def update_ad_probability_click():
    ...
    dbCon = MySQLdb.connect('localhost', 'root', 'notpassword', 'outbrain')
    query = 'select ad_id from promoted_content;'
    ...
    query_template = 'select count(*) from train_data where validation_data = 0 and ad_id = '
    click_query_template = 'select count(*) from train_data where validation_data = 0 and clicked = 1 and ad_id = '
    ...
    update_query_temp = 'update promoted_content set '
    for ad in ad_ids:
        dbCon = MySQLdb.connect('localhost', 'root', 'notpassword', 'outbrain')
        #print(ad[0])
...
def update_feature_probabilities_ad_clicked():
    truncate_table('ad_country_probabilities')
    truncate_table('ad_document_probabilities')
    truncate_table('ad_platform_probabilities')
    truncate_table('ad_state_probabilities')
    truncate_table('ad_dma_probabilities')
    print('Updating feature probabilities....')
    print('Dropping indexes for quick load')
    drop_index('ad_country_probabilities', ['adconindex'])
    drop_index('ad_document_probabilities', ['adpind', 'adpdoind', 'adpdoaind'])
    drop_index('ad_platform_probabilities', ['appind', 'appplind'])
    drop_index('ad_state_probabilities', ['adconstateindex'])
    drop_index('ad_dma_probabilities', ['adcsdma'])
    ...
    #insert_ad_doc_temp = 'insert into ad_document_map values ('ad_count','ad_click_probability'
    #insert_ad_platform_temp = 'insert into ad_platform_map values ('
    update_ad_clicked_document_type_temp = 'update promoted_content set clicked_in_documents = '
    ...
    for ad_id_row in ad_ids:
        ...
        if len(document_data) > 5000:
            write_to_db('ad_document_probabilities', document_data, ad_document_header)
            write_to_db('ad_platform_probabilities', platform_data, ad_platform_header)
            write_to_db('ad_country_probabilities', country_data, ad_country_header)
            write_to_db('ad_state_probabilities', state_data, ad_state_header)
            write_to_db('ad_dma_probabilities', dma_data, ad_state_dma_header)
            document_data = []
            platform_data = []
            country_data = []
            state_data = []
        ...
        # for cv
        clicked_ad_docs = np.array(read_from_table('train_data', {'ad_id': str(ad_id), 'clicked': '1', 'validation_data':'0'},
        ...
        if len(clicked_ad_docs) > 0:# is not None:
            ...
            document_data.append([ad_id, str(-1), str((1)/(advertised_count + len(advertised_documents))), str(0),
                                  str(int(advertised_count/len(advertised_documents))), str(1/(int(advertised_count/len(advertised_documents)) + 5))])

            for platform_id in platform_map:
                platform_data.append([ad_id, str(platform_id), str((1)/(advertised_count + 3)),str(0), str(int(advertised_count/3)), str(1/((advertised_count/3) + 5))])

            country_data.append([ad_id, str('dummy'), str(1/(advertised_count + len(advertised_countries)) ),
                                     str(0), str(0), str(advertised_states_count), str(int(advertised_count/len(advertised_countries))), str(1/((advertised_count/len(advertised_countries)) + 5))])

            state_data.append([ad_id, str('dummy'), str('dummy'), str(1/(advertised_count + advertised_states_count)),
                                       str(0), str(0),
                                       str(advertised_dmas_count),str(int(advertised_count/advertised_states_count)),str(1/((advertised_count/advertised_states_count) + 5))])

            dma_data.append([ad_id, str('dummy'), str('dummy'),str('dummy'),
                             str(1/(advertised_count + advertised_dmas_count)),
                             str(0),str(int(advertised_count/advertised_dmas_count)),str(1/((advertised_count/advertised_dmas_count) + 5))])
    ...
    create_index('ad_country_probabilities', {'adconindex': ['ad_id', 'country']})
    create_index('ad_document_probabilities',
                 {'adpind': ['ad_id', 'document_id'], 'adpdoind': ['document_id'], 'adpdoaind': ['ad_id']})
    create_index('ad_platform_probabilities', {'appind': ['ad_id', 'platform'], 'appplind': ['ad_id']})
    create_index('ad_state_probabilities', {'adconstateindex': ['ad_id', 'country', 'state']})
    create_index('ad_dma_probabilities', {'adcsdma': ['ad_id', 'country', 'state', 'dma']})
...
def update_ad_publisher_source_probabilities():
    truncate_table('ad_publisher_probabilities')
    truncate_table('ad_publisher_source_probabilities')
    print('update_ad_publisher_source_probabilities')
    drop_index('ad_publisher_probabilities',['adppp','adpppa'])
    drop_index('ad_publisher_source_probabilities', ['adpspps', 'adpsppsa','adpsadid'])
    ...
    ad_count = {}
    ad_count_rows = np.array(read_from_table('promoted_content', {'ad_id': None}, ['ad_id','click_count']))
    ...
    for publisher_item in publisher_ids:
        ...
        for source_ in source_ids:
            ...
            for doc_row in doc_publisher_source_rows:
                ...
                ad_doc_rows = np.array(read_from_table('ad_document_probabilities',
        ...
        write_to_db('ad_publisher_probabilities', publisher_data, publisher_prob_header)
        write_to_db('ad_publisher_source_probabilities', publisher_source_data, pub_source_prob_header)
        publisher_data = []
        publisher_source_data = []

    create_index('ad_publisher_probabilities', {'adppp':['publisher_id'], 'adpppa':['publisher_id','ad_id']})
    create_index('ad_publisher_source_probabilities', {'adpspps':['publisher_id', 'source_id'], 'adpsppsa':['source_id', 'publisher_id', 'ad_id'], 'adpsadid':['ad_id']})
...
def update_documents_meta_clicked():
    ...
    documents = np.array(read_unique_from_table('ad_document_probabilities', ['document_id']))
...
def update_advertiser_clicked():
    truncate_table('advertiser_probabilities')
    print('update_advertiser_clicked')
    advertisers  = np.array(read_unique_from_table('promoted_content', ['advertiser_id']))
    drop_index('advertiser_probabilities', ['advertiseridsap'])
    ...
    for row in advertisers:
        ...
        advertiser_rows  = np.array(read_from_table('promoted_content',
        ...
        if len(advertiser_data) > 100:
            write_to_db('advertiser_probabilities', advertiser_data, advertiser_header)
            advertiser_data = []

    write_to_db('advertiser_probabilities', advertiser_data, advertiser_header)
    create_index('advertiser_probabilities', {'advertiseridsap':['advertiser_id']})

def update_advertiser_campaign_clicked():
    truncate_table('advertiser_campaign_probabilities')
    print('update_advertiser_campaign_clicked')
    drop_index('advertiser_campaign_probabilities', ['adcprob'])

    advertiser_campaigns = np.array(read_unique_from_table('promoted_content', ['advertiser_id','campaign_id']))
    ...
    for advertiser in advertiser_campaign_dict:
        ...
        for campaign in campaigns:
            advertiser_camp_rows = np.array(read_from_table('promoted_content',
        ...
        if len(advertiser_campaign_data) > 100:
            write_to_db('advertiser_campaign_probabilities', advertiser_campaign_data, campaign_header)
            advertiser_campaign_data = []

    write_to_db('advertiser_campaign_probabilities', advertiser_campaign_data, campaign_header)
    create_index('advertiser_campaign_probabilities', {'adcprob': ['advertiser_id', 'campaign_id']})

def update_publisher_source_advertiser_campaign_probability():
    truncate_table('publisher_campaign_probabilities')
    truncate_table('source_campaign_probabilities')
    print('update_publisher_source_advertiser_campaign_probability')

    drop_index('publisher_campaign_probabilities', ['advcampub', 'pubpcp'])
    drop_index('source_campaign_probabilities', ['advcampubsource','pubsourceps'])
    ...
    advertiser_camp_click_count = np.array(read_from_table('advertiser_campaign_probabilities', {'advertiser_id': None},
                                                           ['advertiser_id', 'campaign_id',
                                                            'number_of_clicks', 'published_number']))
    advertiser_campaign_source_count = {}
    advertiser_campaign_source_adv_count = {}
    ...
    for advertiser_campaign in advertiser_camp_click_count:
        ...
        if ad_id in advertiser_campaign_source_count.keys():
            ...
            advertiser_campaign_source_adv_count[ad_id][campaign_id] = int(advertiser_campaign[3])
        else:
            advertiser_campaign_source_count[ad_id] = {}
            advertiser_campaign_source_count[ad_id][campaign_id] = int(advertiser_campaign[2])
            advertiser_campaign_source_adv_count[ad_id] = {}
            advertiser_campaign_source_adv_count[ad_id][campaign_id] = int(advertiser_campaign[3])
    ...
    for advertiser in advertiser_campaign_source_count:
        ...
        for campaign in campaigns:
            ...
            publisher_count = {}
            publisher_source_adv_count = {}
            publisher_adv_count = {}
            for ad in advertiser_camp_rows:
                ad_id = str(ad[0])
                ad_publsiher_source = np.array(read_from_table('ad_publisher_source_probabilities',
                                                                {'ad_id': "'" + ad_id + "'"},
                                                                ['publisher_id','source_id','publisher_source_count','ad_count']))
                for row in ad_publsiher_source:
                    ...
                    if publisher_id in publisher_source_count.keys():
                        ...
                        publisher_source_count[publisher_id][source_id] = int(row[2])
                        publisher_source_adv_count[publisher_id] = {}
                        publisher_source_adv_count[publisher_id][source_id] = int(row[3])
            ...
            for publisher in publisher_source_count:
                publisher_campaign_data.append([advertiser, campaign, publisher,
                                                ...
                                                str(publisher_count[publisher]), str(publisher_adv_count[publisher]),
                                                str((publisher_count[publisher] + 1)/(publisher_adv_count[publisher] + 5))])
                for source in publisher_source_count[publisher]:
                    source_campaign_data.append([advertiser, campaign, publisher, source,
                                                 ...
                                                 str(publisher_source_adv_count[publisher][source]),
                                                 str((publisher_source_count[publisher][source] + 1)/(publisher_source_adv_count[publisher][source] + 5))])


            if len(source_campaign_data) > 1000:
                write_to_db('publisher_campaign_probabilities', publisher_campaign_data, publisher_camp_header)
                write_to_db('source_campaign_probabilities', source_campaign_data, source_table_header)
                publisher_campaign_data = []
                source_campaign_data = []
    ...
    create_index('publisher_campaign_probabilities',
                 {'advcampub': ['advertiser_id', 'campaign_id', 'publisher_id'], 'pubpcp':['publisher_id']})
    create_index('source_campaign_probabilities', {'advcampubsource': ['advertiser_id', 'campaign_id', 'publisher_id', 'source_id'], 'pubsourceps':['publisher_id', 'source_id']})

def update_ad_hour_probability():
    truncate_table('ad_hour_probability')
    drop_index('ad_hour_probability',['adhour'])
    ...
    ad_ids = np.array(read_unique_from_table('promoted_content', ['ad_id']))
    ...
    for ad_row in ad_ids:
        ...
        # for cv data
        clicked_ads = read_from_table('train_data', {'ad_id': str(ad_id), 'clicked': '1',  'validation_data':'0'},['event_hour'])

        # for all data
        advertised_ads = read_from_table('train_data', {'ad_id': str(ad_id),  'validation_data':'0'},['event_hour'])
        ...
        if len(ad_hour_data) > 10000:
            write_to_db('ad_hour_probability', ad_hour_data, ad_hour_header)
            ad_hour_data = []

    write_to_db('ad_hour_probability', ad_hour_data, ad_hour_header)
    create_index('ad_hour_probability', {'adhour': ['ad_id', 'hour_duration']})


def update_ad_traffic_source_probability():
    truncate_table('ad_traffic_source_probability')
    drop_index('ad_traffic_source_probability', ['adtrsp'])
    ...
    for ad_row in ad_ids:
        ...
        # for cv
        clicked_ads = read_from_table('train_data', {'ad_id': str(ad_id), 'clicked': '1', 'validation_data':'0'}, ['traffic_source'])
        ...
        if len(ad_traffic_source_data) > 10000:
            write_to_db('ad_traffic_source_probability', ad_traffic_source_data, ad_tr_source_header)
            ad_traffic_source_data = []

    write_to_db('ad_traffic_source_probability', ad_traffic_source_data, ad_tr_source_header)
    create_index('ad_traffic_source_probability', {'adtrsp': ['ad_id', 'traffic_source']})

def update_ad_day_probability():
    truncate_table('ad_day_probability')
    drop_index('ad_day_probability', ['adday'])
    print('update_ad_day_probability')
    ad_ids = np.array(read_unique_from_table('promoted_content', ['ad_id']))
    ...
    for ad_row in ad_ids:
        ...
        # for cv
        clicked_ads = read_from_table('train_data', {'ad_id': str(ad_id), 'clicked': '1', 'validation_data':'0'}, ['event_day'])
            ...
            for day in day_click_count:
                ...
                ad_hour_data.append([ad_id, str(day),
                                 ...
                                 str(day_click_count[day]), str(advertised_day_counts.get(day, 0)),
                                     ...
                                     str(train_advertised_day_counts.get(day, 0)), str((day_click_count[day]+1)/(train_advertised_day_counts.get(day, 0)+5))])
        ...
        # These probabilities won't sum to one - making some naive predictions for day 13 and 14
            ...
            if max_probability > 0.8: # 0.8 is by intuition
                if max_day < 10: # for cv
                #if max_day < 11: # if the spike in probability is not on 11th or 12th day
                    ...
                    ad_hour_data.append([ad_id, str(13),
                                         str(((non_max_sum/adv_days) + 1)/(total_clicks_for_ad + len(advertised_day_counts)) ), # 12 is number of non max days excluding day 13 and 14
                                         str(0), str(advertised_day_counts.get(13, 0)), str(advertised_day_counts.get(13, 0)/advertised_count),
                                         str(train_advertised_day_counts.get(13, 0)),
                                         str(((non_max_sum/adv_days) + 1) / (train_advertised_day_counts.get(13, 0) + 5))]) # we don't know the count
                    ad_hour_data.append([ad_id, str(14),
                                         str(((non_max_sum/adv_days) + 1)/(total_clicks_for_ad + len(advertised_day_counts))),
                                         str(0), str(advertised_day_counts.get(14, 0)), str(advertised_day_counts.get(14, 0)/advertised_count),
                                         str(train_advertised_day_counts.get(14, 0)),
                                         str(((non_max_sum / adv_days) + 1) / (train_advertised_day_counts.get(14, 0) + 5))])
                    # for cv of day 12
                    if 12 in train_advertised_day_counts.keys():
                        adv_days -= 1
                    ad_hour_data.append([ad_id, str(99),
                                     str(((non_max_sum / adv_days) + 1) / (total_clicks_for_ad + 15)),
                                     str(0), str(advertised_day_counts.get(12, 0)), str(advertised_day_counts.get(12, 0)/advertised_count),
                                         str(train_advertised_day_counts.get(12, 0)),
                                         str(((non_max_sum / adv_days) + 1) / (train_advertised_day_counts.get(11, 0) + 5))
                                         ])
                else: # if spike is in day 11 or 12
                    if max_day == 10: # for cv
                        if (day_click_count[11]/total_clicks_for_ad) >= 0.1:
                            ad_hour_data.append([ad_id, str(13),
                                                 str(((day_click_count[12] / 1.5) + 1) / (total_clicks_for_ad + len(advertised_day_counts))),
                            ...
                            ad_hour_data.append([ad_id, str(14),
                                                 str(((day_click_count[12] / 2) + 1) / (total_clicks_for_ad + len(advertised_day_counts))),  #
                                                 str(0) , str(advertised_day_counts.get(14, 0)), str(advertised_day_counts.get(14, 0)/advertised_count),
                                                 str(train_advertised_day_counts.get(14, 0)),
                                                 str((day_click_count[12]  + 1) / (
                                                     train_advertised_day_counts.get(12, 0) + 5))
                            ...
                            # for cv
                            ad_hour_data.append([ad_id, str(99),
                                                 str(((day_click_count[11] / 2) + 1) / (total_clicks_for_ad + len(advertised_day_counts))),  #
                                                 str(0), str(advertised_day_counts.get(12, 0)), str(advertised_day_counts.get(12, 0)/advertised_count),
                                                 str(train_advertised_day_counts.get(12, 0)),
                                                 str((day_click_count[11]  + 1) / (
                                                     train_advertised_day_counts.get(11, 0) + 5))
                                                 ])
                        else:
                            ad_hour_data.append([ad_id, str(13),
                                                 str(((day_click_count[12] / 1.05)+ 1)/ (total_clicks_for_ad + len(advertised_day_counts))),  # using day 12 probability with penalty
                                                 str(0), str(advertised_day_counts.get(13, 0)), str(advertised_day_counts.get(13, 0)/advertised_count),
                                                 str(train_advertised_day_counts.get(13, 0)),
                                                 str((day_click_count[12]  + 1) / (
                                                     train_advertised_day_counts.get(12, 0) + 5))
                            ...
                            ad_hour_data.append([ad_id, str(14),
                                                 str(((day_click_count[12] / 1.1)+ 1)/ (total_clicks_for_ad + len(advertised_day_counts))),  #
                                                 str(0), str(advertised_day_counts.get(14, 0)), str(advertised_day_counts.get(14, 0)/advertised_count),
                                                 str(train_advertised_day_counts.get(14, 0)),
                                                 str((day_click_count[12]  + 1) / (
                                                     train_advertised_day_counts.get(12, 0) + 5))
                            ...
                            # for cv
                            ad_hour_data.append([ad_id, str(99),
                                                 str(((day_click_count[11] / 1.1) + 1) / (total_clicks_for_ad + len(advertised_day_counts))),  #
                                                 str(0), str(advertised_day_counts.get(12, 0)), str(advertised_day_counts.get(12, 0)/advertised_count),
                                                 str(train_advertised_day_counts.get(12, 0)),
                                                 str((day_click_count[11]  + 1) / (
                                                     train_advertised_day_counts.get(11, 0) + 5))
                        ...
                        ad_hour_data.append([ad_id, str(13),
                                             str(((day_click_count[12] / 3) + 1) / (total_clicks_for_ad + len(advertised_day_counts))), # reducing the day 12, count by factor of 3 to avoid overfitting. This is not always the case. Some times the count could go up.
                                             str(0), str(advertised_day_counts.get(13, 0)), str(advertised_day_counts.get(13, 0)/advertised_count),
                                             str(train_advertised_day_counts.get(13, 0)),
                                             str((day_click_count[12] + 1) / (
                                                 train_advertised_day_counts.get(12, 0) + 5))
                        ...
                        ad_hour_data.append([ad_id, str(14),
                                             str(((day_click_count[12] / 5) + 1) / (total_clicks_for_ad + len(advertised_day_counts))), # reducing the day 12, count by factor of 5 for further depreciation to avoid overfitting
                                             str(0), str(advertised_day_counts.get(14, 0)), str(advertised_day_counts.get(14, 0)/advertised_count),
                                             str(train_advertised_day_counts.get(14, 0)),
                                             str((day_click_count[12]  + 1) / (
                                                 train_advertised_day_counts.get(12, 0) + 5))
                        ...
                        #cv
                        ad_hour_data.append([ad_id, str(99),
                                             str(((day_click_count[11] / 3) + 1) / (total_clicks_for_ad + len(advertised_day_counts))),
                ...
                ad_hour_data.append([ad_id, str(13),
                                     str((((day_click_count[6] + day_click_count[11] + day_click_count[12])/3) + 1) / (total_clicks_for_ad + len(advertised_day_counts))),
                                     str(0), str(advertised_day_counts.get(13, 0)), str(advertised_day_counts.get(13, 0)/advertised_count),
                                     str(train_advertised_day_counts.get(13, 0)),
                                     str((((day_click_count[6] + day_click_count[11] + day_click_count[12])) + 1) / (
                                         train_advertised_day_counts.get(6, 0) + train_advertised_day_counts.get(11, 0) + train_advertised_day_counts.get(12, 0) + 5))
                                     ])  # we don't know the count
                ad_hour_data.append([ad_id, str(14),
                                     str((((day_click_count[0] + day_click_count[7] + day_click_count[11] + day_click_count[12])/4) + 1) / (total_clicks_for_ad + len(advertised_day_counts))),
                                     str(0), str(advertised_day_counts.get(14, 0)), str(advertised_day_counts.get(14, 0)/advertised_count),
                                     str(train_advertised_day_counts.get(14, 0)),
                                     str((
                                         ((day_click_count[0] + day_click_count[7]+ day_click_count[11] + day_click_count[12]) ) + 1) / (
                                         train_advertised_day_counts.get(0, 0) + train_advertised_day_counts.get(7, 0)
                                         + train_advertised_day_counts.get(11,0) + train_advertised_day_counts.get(12, 0) + 5))
                                     ])
                # This is needed only for cross_valid_data - for day 12
                ad_hour_data.append([ad_id, str(99),
                                     str((((day_click_count[5] + day_click_count[10] + day_click_count[
                                         11]) / 3) + 1) / (total_clicks_for_ad + len(advertised_day_counts))),
                                     str(0), str(advertised_day_counts.get(12, 0)), str(advertised_day_counts.get(12, 0)/advertised_count),
                                     str(train_advertised_day_counts.get(12, 0)),
                                     str((
                                             ((day_click_count[5] + day_click_count[10] + day_click_count[11] ) ) + 1) / (
                                             train_advertised_day_counts.get(5, 0)
                                             + train_advertised_day_counts.get(10, 0) + train_advertised_day_counts.get(
                                                 11, 0) + 5))
                                     ])

        if len(ad_hour_data) > 10000:
            write_to_db('ad_day_probability', ad_hour_data, ad_hour_header)
            ad_hour_data = []

    write_to_db('ad_day_probability', ad_hour_data, ad_hour_header)
    create_index('ad_day_probability', {'adday': ['ad_id', 'event_day']})


def update_day_hour_probability():
    truncate_table('ad_day_hour_probability')
    drop_index('ad_day_hour_probability', ['dhprb'])
    print('update_day_hour_probability')
    hour_increment = 4  # choosing this by intuition. Need to vary and experiment
    number_of_hours_in_day = 24
    ad_ids = np.array(read_unique_from_table('promoted_content', ['ad_id']))
    ...
    for ad_row in ad_ids:
        ...
        # for cv
        clicked_ads = read_from_table('train_data', {'ad_id': str(ad_id), 'clicked': '1', 'validation_data':'0'}, ['event_day','event_hour'])

        advertised_ads = read_from_table('train_data', {'ad_id': str(ad_id), 'validation_data': '0'},
        ...
        for day in ad_day_hour_count:
            for hour_window in ad_day_hour_count[day]:
                ad_day_hour_data.append([ad_id, str(day),str(hour_window),
                                ...
                                str((ad_day_hour_count[day][hour_window] + 1)/(adv_day_hour_count[day][hour_window] + 5))])

        if len(ad_day_hour_data) > 10000:
            write_to_db('ad_day_hour_probability', ad_day_hour_data, ad_day_hour_header)
            ad_day_hour_data = []

    write_to_db('ad_day_hour_probability', ad_day_hour_data, ad_day_hour_header)
    create_index('ad_day_hour_probability', {'dhprb': ['ad_id', 'event_day', 'event_hour']})
...
def update_ad_pair_data1():
    ...
    drop_index('ad_pair_data_cv', ['adpairind', 'adpaircd', 'adpairdd'])
    ...
    for display_id in test_disp_ids:
        ...
        if len(str(display_id)) > 0:
            ad_disp_rows = np.array(read_from_table('train_data', {'display_id': str(display_id), 'validation_data':'0'},['ad_id','clicked']))
            ad_ids = []
            clicked_ad = 0
            if len(ad_disp_rows) > 0 and len(ad_disp_rows[0]) > 0:
    ...
    for ad in ad_display_dict:
        ...
        if len(ad_pair_data) > 100:
            write_to_db('ad_pair_data_cv', ad_pair_data, ad_pair_header)
            ad_pair_data = []

    write_to_db('ad_pair_data_cv', ad_pair_data, ad_pair_header)
    create_index('ad_pair_data_cv', {'adpairind': ['ad1_id', 'ad2_id'], 'adpaircd':['ad1_clicked_ad2_disp'], 'adpairdd':['ad1_ad2_disp']})


def update_campaign_advertiser_pair_data():
    truncate_table('campaign_pair_data')
    truncate_table('advertiser_pair_data')
    ...
    ads = np.array(read_from_table('promoted_content', {'ad_id': None},
    ...
    for advertiser in advertiser_ad_ids:
        ...
        for ad in ads:
            ...
            campaign1 = ad_campaign_id[ad1_id]
            ad_pair_rows = read_from_table('ad_pair_data_cv', {'ad1_id': str(ad1_id)}, ['ad1_id',  'ad2_id',  'ad1_clicked_ad2_disp',  'ad1_ad2_disp'])
        ...
        if len(campaign_pair_data) > 15000:
            write_to_db('advertiser_pair_data', advertiser_pair_data, advertiser_pair_header)
            write_to_db('campaign_pair_data', campaign_pair_data, campaign_pair_header)
            campaign_pair_data = []
            advertiser_pair_data = []

    write_to_db('advertiser_pair_data', advertiser_pair_data, advertiser_pair_header)
    write_to_db('campaign_pair_data', campaign_pair_data, campaign_pair_header)
    create_index('campaign_pair_data', {'campaign1ind':['advertiser1_id', 'campaign1_id'],'campaignpairind':['advertiser1_id', 'campaign1_id', 'advertiser2_id', 'campaign2_id']})
    create_index('advertiser_pair_data', {'advertiser1ind':['advertiser1_id'],'advertiserpairind':['advertiser1_id', 'advertiser2_id']})
...
def update_advertiser_feature_probabilities():
    ...
    tables = ['advertiser_country_probabilities','advertiser_state_probabilities', 'advertiser_dma_probabilities',
              'advertiser_day_probabilities','advertiser_hour_probabilities','advertiser_day_hour_probabilities',
              'campaign_day_probabilities','campaign_day_hour_probabilities','campaign_hour_probabilities',
              'campaign_country_probabilities', 'campaign_state_probabilities', 'campaign_dma_probabilities']
    ...
    table_indexes = {'advertiser_country_probabilities' :{'adc':['advertiser_id', 'country']},
                     'advertiser_state_probabilities':{'ads':['advertiser_id','country', 'state']},
                     'advertiser_dma_probabilities':{'adcdm':['advertiser_id', 'country', 'state', 'dma']},
                     'advertiser_day_probabilities':{'adday':['advertiser_id', 'event_day']},
                     'advertiser_hour_probabilities':{'adh':['advertiser_id', 'event_hour']},
                     'advertiser_day_hour_probabilities':{'addh':['advertiser_id', 'event_day', 'event_hour']},
                     'campaign_day_probabilities':{'adday':['advertiser_id', 'campaign_id', 'event_day']},
                     'campaign_day_hour_probabilities':{'addh':['advertiser_id', 'campaign_id', 'event_day', 'event_hour']},
                     'campaign_hour_probabilities':{'adh':['advertiser_id', 'campaign_id', 'event_hour']},
                     'campaign_country_probabilities':{'adcc':['advertiser_id', 'campaign_id', 'country']},
                     'campaign_state_probabilities':{'adcs':['advertiser_id', 'campaign_id', 'country', 'state']},
                     'campaign_dma_probabilities':{'adcdm':['advertiser_id', 'campaign_id', 'country', 'state', 'dma']}}

    for table in table_indexes:
        drop_index(table, table_indexes[table].keys())

    table_headers = {'advertiser_country_probabilities' :['advertiser_id', 'country', 'click_count', 'advertised_count', 'click_probability'],
                     'advertiser_state_probabilities':['advertiser_id','country', 'state',  'click_count', 'advertised_count', 'click_probability'],
                     'advertiser_dma_probabilities':['advertiser_id', 'country', 'state', 'dma', 'click_count', 'advertised_count', 'click_probability'],
                     'advertiser_day_probabilities':['advertiser_id', 'event_day', 'click_count', 'advertised_count', 'click_probability'],
                     'advertiser_hour_probabilities':['advertiser_id', 'event_hour', 'click_count', 'advertised_count', 'click_probability'],
                     'advertiser_day_hour_probabilities':['advertiser_id', 'event_day', 'event_hour', 'click_count', 'advertised_count', 'click_probability'],
                     'campaign_day_probabilities':['advertiser_id', 'campaign_id', 'event_day', 'click_count', 'advertised_count', 'click_probability'],
                     'campaign_day_hour_probabilities':['advertiser_id', 'campaign_id', 'event_day', 'event_hour', 'click_count', 'advertised_count', 'click_probability'],
                     'campaign_hour_probabilities':['advertiser_id', 'campaign_id', 'event_hour', 'click_count', 'advertised_count', 'click_probability'],
                     'campaign_country_probabilities':['advertiser_id', 'campaign_id', 'country', 'click_count', 'advertised_count', 'click_probability'],
                     'campaign_state_probabilities':['advertiser_id', 'campaign_id','country', 'state',  'click_count', 'advertised_count', 'click_probability'],
                     'campaign_dma_probabilities':['advertiser_id', 'campaign_id', 'country', 'state', 'dma', 'click_count', 'advertised_count', 'click_probability']}
    ...
    table_data = {'advertiser_country_probabilities': [], 'advertiser_state_probabilities': [],
                  'advertiser_dma_probabilities': [],
                  'advertiser_day_probabilities': [], 'advertiser_hour_probabilities': [],
                  'advertiser_day_hour_probabilities': [],
                  'campaign_day_probabilities': [], 'campaign_day_hour_probabilities': [],
                  'campaign_hour_probabilities': [],
                  'campaign_country_probabilities': [], 'campaign_state_probabilities': [],
                  'campaign_dma_probabilities': []}
    ...
    for advertiser in advertiser_ad_ids:
        ...
        for ad in ads:
            ...
            countries = np.array(read_from_table('ad_country_probabilities', {'ad_id': ad_id},
            ...
            states = np.array(read_from_table('ad_state_probabilities', {'ad_id': ad_id},
            ...
            dmas = np.array(read_from_table('ad_dma_probabilities', {'ad_id': ad_id},
            ...
            days = np.array(read_from_table('ad_day_probability', {'ad_id': ad_id},
            ...
            hours = np.array(read_from_table('ad_hour_probability', {'ad_id': ad_id},
            ...
            day_hours = np.array(read_from_table('ad_day_hour_probability', {'ad_id': ad_id},
            ...
            if '12' in day_click_count:
                day_click_count['12'] = int((day_click_count['11'] + day_click_count['5'])/2)
                day_count['12'] = int((day_count['11'] + day_count['5']) / 2)
                day_click_count['99'] = int((day_click_count['11'] + day_click_count['5']) / 2)
                day_count['99'] = int((day_count['11'] + day_count['5']) / 2)
        ...
        for country in ad_country_count:
            table_data['advertiser_country_probabilities'].append([str(advertiser), str(country),
            ...
            if country in ad_state_count:
                for state in ad_state_count[country]:
                    table_data['advertiser_state_probabilities'].append([str(advertiser), str(country), str(state),
                    ...
                    if country in ad_dma_count and state in ad_dma_count[country]:
                        for dma in ad_dma_count[country][state]:
                            table_data['advertiser_dma_probabilities'].append([str(advertiser), str(country), str(state), str(dma),
        ...
        for day in ad_day_count:
            table_data['advertiser_day_probabilities'].append([str(advertiser), str(day),
        ...
        for day in ad_dh_click_count:
            for h in ad_dh_click_count[day]:
                table_data['advertiser_day_hour_probabilities'].append([str(advertiser), str(day), str(h),
                                                                        str(ad_dh_click_count[day][h]),
                                                                        str(ad_dh_count[day][h]),
                                                                        str((ad_dh_click_count[day][h] + 1) / (
                                                                            ad_dh_count[day][h] + 5))])
        for h in ad_hour_click_count:
            table_data['advertiser_hour_probabilities'].append([str(advertiser), str(h),
        ...
        for campaign in campaign_country_click_count.keys():
            for country in campaign_country_count[campaign]:
                table_data['campaign_country_probabilities'].append([str(advertiser), str(campaign), str(country),
                ...
                if country in campaign_state_count[campaign]:
                    for state in campaign_state_count[campaign][country]:
                        table_data['campaign_state_probabilities'].append(
                        ...
                        if country in campaign_dma_count[campaign] and state in campaign_dma_count[campaign][country]:
                            for dma in campaign_dma_count[campaign][country][state]:
                                table_data['campaign_dma_probabilities'].append(
            ...
            for day in campaign_day_count[campaign]:
                table_data['campaign_day_probabilities'].append([str(advertiser), str(campaign), str(day),
            ...
            for day in campaign_dh_click_count[campaign]:
                for h in campaign_dh_click_count[campaign][day]:
                    table_data['campaign_day_hour_probabilities'].append(
            ...
            for h in campaign_hour_click_count[campaign]:
                table_data['campaign_hour_probabilities'].append([str(advertiser), str(campaign), str(h),
...
#update_ad_pair_data1() # NA ~ 2 hr

## TODO - change for cv
#4
#update_ad_probability_click() # -cv

#5
#update_feature_probabilities_ad_clicked() #- cv ~ 5 hr


#6
#update_ad_publisher_source_probabilities() # NA - rerun ~120 mins
#update_advertiser_clicked() # NA - rerun ~6s
#update_advertiser_campaign_clicked() # NA - rerun ~ 2 min

#update_ad_hour_probability() # all ~ 20 mins
#update_ad_traffic_source_probability() # all ~ 20 mins
#update_ad_day_probability() # all ~ 2 hrs
#update_day_hour_probability() # all ~ 4 hrs
...
# 8
#update_campaign_advertiser_pair_data()# ~ 30 mins
